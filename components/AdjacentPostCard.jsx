...
const AdjacentPostCard = ({ post, position }) => (
  <>
    ...
    <div className="flex flex-col rounded-lg p-4 items-center justify-center absolute w-full h-full">
      <p className="text-white text-shadow font-semibold text-xs">{moment(post.createdAt).locale('id').format('DD MMMM YYYY')}</p>
      <p className="text-white text-shadow font-semibold text-base lg:text-2xl text-center">{post.title}</p>
    </div>
    <Link href={`/posts/${post.slug}`}><span className="z-10 cursor-pointer absolute w-full h-full" /></Link>
    {position === 'LEFT' && (
